 ...
 * */
void heapsort(char *a, int n, int (*cmp)(char, char))
{
	 ...
	 * Первый этап: превратить дерево в такое, где всякая цепочка от корня до любого конечного элемента упорядочена по убыванию.
	 * */
	for( k = 1; k<n; k++ )
	{
	...
	for( k = n - 1; k>0; k-- )
	{
		...
		for( position = 0; position<(k - 1); child_1 = 2*position + 1, child_2 = 2*position + 2)
		{
			...
			if( cmp(a[child_2], a[0])>0 )
			{
				if( cmp(a[child_1], a[child_2])>0 )
				{
					position = child_1;
				} else
				{
